...
class VerificationService {
  private async uploadIDCard(userId: string, uri: string, side: 'front' | 'back'): Promise<string> {
    try {
  ...
  async verifyUser(userId: string, frontUri: string, backUri: string): Promise<{ success: boolean; message: string; errors?: string[] }> {
    try {
      ...
      const frontImageData = await this.uploadIDCard(userId, frontUri, 'front');
      const backImageData = await this.uploadIDCard(userId, backUri, 'back');
